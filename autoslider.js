...
function initSlider(wrapper) {
    ...
    if (config.aspectRatio === "auto") {
        ...
        if (firstImg) {
            const applyAutoRatio = () => {
                const natWidth = firstImg.naturalWidth;
                const natHeight = firstImg.naturalHeight;
                if (natWidth && natHeight) {
                    const ratio = (natWidth * config.showItems) / natHeight;
                    container.style.aspectRatio = ratio.toString();
                }
            };
            firstImg.complete ? applyAutoRatio() : firstImg.addEventListener('load', applyAutoRatio);
        }
    } else {
    ...
    const slideNext = (isAuto = false) => {
        ...
        if (currentIndex >= maxIndex) {
            ...
            // ステップ数分進めるが、最大値（空欄防止）を超えないようにする
